...
class pkg():
    ...
    def q1(self):
        ...
        self.args['roles']['yubin'].add('paladin') ## change
        return self.args['roles']['yubin']
    
    def q2(self):
        "將 'warlock','priest' 加到 shaohua 用的角色, 提示 update"
        q=['warlock','priest']
        self.args['roles']['shaohua'].update(q) ## change
        return self.args['roles']['shaohua']
    
    def q3(self):
        "依據使用角色數目由大到小排序使用者, 提示sort"
        ans = sorted(self.args['roles'].keys(), key =lambda x:len(self.args['roles'][x]), reverse=True ) ## change
        return ans
    
    def q4(self):
        "將 'warrior'角色從 yubin 的使用角色中刪除, 提示 discard "
        self.args['roles']['yubin'].discard('warrior') ## change
        return self.args['roles']['yubin']

    def q5(self):
        "請列出 yubin 與 shaohua 兩人加起來使用的所有角色, 提示 union"
        ans = self.args['roles']['yubin'].union(self.args['roles']['shaohua']) ## change
        return ans

    def q6(self):
        "請列出 yubin 與 shaohua 都有使用的角色, 提示 intersection"
        ans = self.args['roles']['yubin'].intersection(self.args['roles']['shaohua']) ## change
        return ans
    
    def q7(self):
        "請列出 yubin 使用但 shaohua 不使用的角色, 提示 difference"
        ans = self.args['roles']['yubin'].difference(self.args['roles']['shaohua']) ## change
        return ans
